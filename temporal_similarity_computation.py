...
def batch_timelist_ground_truth(valiortest=None):
    ...
    all_time_list_int = np.load(str(config.shuffle_time_file), allow_pickle=True)
    if valiortest == 'vali':
        time_list_int = all_time_list_int[config.train_set_size:config.vali_set_size]  # [15000:20000]
    elif valiortest == 'test':
        time_list_int = all_time_list_int[config.vali_set_size:config.test_set_size]  # [20000:50000]
    elif valiortest == 'train':
        time_list_int = all_time_list_int[0:config.train_set_size]  # [0:15000]

    if valiortest == 'vali':
        sample_list = all_time_list_int[config.train_set_size:config.vali_set_size]
    elif valiortest == 'test':
        sample_list = all_time_list_int[config.vali_set_size:config.test_set_size]
    elif valiortest == 'train':
        sample_list = all_time_list_int[0:config.train_set_size]  # [0:15000]

    last_num = len(sample_list) % 50
    if last_num != 0:
        timelist_distance(len(sample_list), sample_list[len(
            sample_list) - last_num:len(sample_list)], time_list_int, valiortest)
    ...
    #         timelist_distance(i, sample_list[i-50:i], time_list_int, valiortest)
...
def merge_timelist_ground_truth(sample_len, valiortest):
    ...
    for i in range(sample_len+1):
        if i != 0 and i % 50 == 0:
            res.append(np.load('./ground_truth/{}/{}/{}_batch_temporal/{}_temporal_distance_{}.npy'.format(dataset,
                       str(config.distance_type), valiortest, str(config.distance_type), str(i))))
    last_num = sample_len % 50
    if last_num != 0:
        res.append(np.load('./ground_truth/{}/{}/{}_batch_temporal/{}_temporal_distance_{}.npy'.format(dataset,
                                                                                                       str(config.distance_type), valiortest, str(config.distance_type), str(sample_len))))

    res = np.concatenate(res, axis=0)
    np.save('./ground_truth/{}/{}/{}_temporal_distance.npy'.format(dataset,
            str(config.distance_type), valiortest), res)
...
def timelist_distance(k, sample_list=[[]], test_list=[[]], valiortest=None):
    ...
    p = './ground_truth/{}/{}/{}_batch_temporal/'.format(dataset, str(config.distance_type), valiortest)
    if not os.path.exists(p):
        os.makedirs(p)
    np.save('./ground_truth/{}/{}/{}_batch_temporal/{}_temporal_distance_{}.npy'.format(dataset,
            str(config.distance_type), valiortest, str(config.distance_type), str(k)), all_dis_list)
...
def NetERP_dis(list_a=[], list_b=[]):
    ...
    for i in range(1, lena + 1):
        for j in range(1, lenb + 1):
            edit[i][j] = min(edit[i - 1][j] + list_a[i-1] - smallest_trajtime, edit[i][j - 1] +
                             list_b[j-1] - smallest_trajtime, edit[i - 1][j - 1] + abs(list_a[i-1] - list_b[j-1]))
...
if __name__ == '__main__':
    ...
    sample_len = batch_timelist_ground_truth(valiortest='train')
    merge_timelist_ground_truth(sample_len=sample_len, valiortest='train')
